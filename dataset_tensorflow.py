...
str2num = load('data/USPTO-50k_no_rxn/USPTO-50k_no_rxn.vocab.txt')
str2num['<mask>'] = len(str2num)
# str2num = {'<pad>':0 ,'H': 1, 'C': 2, 'N': 3, 'O': 4, 'F': 5, 'S': 6, 'Cl': 7, 'P': 8, 'Br':  9,
#          'B': 10,'I': 11,'Si':12,'Se':13,'<unk>':14,'<mask>':15,'<global>':16} # token list
...
class Graph_Bert_Dataset(object):
    ...
    def get_data(self):
        ...
        data1 = data[:int(len(data)*0.9)]
        data2 = data[int(len(data)*0.9):]

        self.dataset1 = tf.data.Dataset.from_tensor_slices(data1.tolist())
        self.dataset1 = self.dataset1.map(self.tf_numerical_smiles).padded_batch(256, padded_shapes=(
            tf.TensorShape([None]), tf.TensorShape([None]) ,tf.TensorShape([None]))).prefetch(50)

        self.dataset2 = tf.data.Dataset.from_tensor_slices(data2.tolist())
    ...
    def numerical_smiles(self, smiles):
        # print(smiles.numpy().decode())

        smiles = smiles.numpy().tolist()
        # atoms_list, adjoin_matrix = smiles2adjoin(smiles,explicit_hydrogens=self.addH)
        # atoms_list = ['<global>'] + atoms_list
        nums_list = [str2num.get(i, str2num['<unk>']) for i in smiles]
    ...
    def tf_numerical_smiles(self, data):
        ...
        x, y, weight = tf.py_function(self.numerical_smiles, [data],
